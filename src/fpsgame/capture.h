...
struct captureservmode : servmode
{
    ...
    void setServMapBases(PyObject *args)
    {
    	int numbases;
    	int temp;
    	PyObject *tuple;
    	...
    	numbases = PyTuple_Size(tuple);

    	loopi(numbases)
    	{
    		tuple_base = PyTuple_GetItem(tuple,i);

    		int ammotype = PyInt_AsLong(PyTuple_GetItem(tuple_base,0));
    		vec o;
    		loopk(3)
    		{
    			temp = PyInt_AsLong(PyTuple_GetItem(tuple_base,k+1));
    			o[k] = max(temp/DMF, 0.0f);
    		}
    		addbase(ammotype>=GUN_SG && ammotype<=GUN_PISTOL ? ammotype : min(ammotype, 0), o);
    	}
        notgotbases = false;
        sendbases();
        loopv(clients) if(clients[i]->state.state==CS_ALIVE) entergame(clients[i]);
    ...
    void parseclientbases(clientinfo *ci, ucharbuf &p)
    {
        int numbases = getint(p);

    	std::vector<PyObject*> args;
    	PyObject *pCn = PyInt_FromLong(ci->clientnum);
    	PyObject *pTuple_bases = PyTuple_New(numbases);
    	PyObject *pTuple_base;

        loopi(numbases)
        {
        	pTuple_base = PyTuple_New(4);
            loopk(4)
            {
            	PyTuple_SetItem(pTuple_base, k, PyInt_FromLong(getint(p)));
            }
            PyTuple_SetItem(pTuple_bases, i, pTuple_base);
        }
...
case N_BASES:
    ...
    capturemode.parseclientbases(ci, p);
    break;
