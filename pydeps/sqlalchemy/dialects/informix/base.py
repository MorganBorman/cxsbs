...
class InformixDialect(default.DefaultDialect):
    ...
    def do_begin(self, connection):
        ...
        cu.execute('SET LOCK MODE TO WAIT 5')
        if self.has_transactions:
#            cu.execute('SET ISOLATION TO DIRTY READ')
    ...
    def get_schema_names(self, connection, **kw):
        s = "select unique(owner) from systables"
        return [row[0] for row in connection.execute(s)]

    def has_table(self, connection, table_name, schema=None):
        currentSchema = schema or self.default_schema_name
        cursor = connection.execute(
                """select tabname from systables where tabname=? and owner=?""",
                table_name, currentSchema)
    ...
    def _get_default_schema_name(self, connection):
			  # LWF - fixed this query to actually return results
			  return connection.scalar('select username from sysmaster:syssessions where sid=dbinfo(\'sessionid\')')
