...
def onAddTagCommand(cn, args):
	...
	try:
		...
		session.add(tag)
		session.commit()
		Player.player(cn).logAction('Added clantag: ' + tagString + ' to group: ' + groupName)
...
def onDelTagCommand(cn, args):
	...
	try:
		...
		if len(removedGroups) == 1:
			Player.player(cn).logAction('Removed clantag: ' + tagString + ' from group: ' + removedGroups[0])
			...
			Player.player(cn).logAction('Removed clantag: ' + tagString + ' from groups: ' + groupNames)
